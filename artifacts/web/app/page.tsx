      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-2">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" className="flex-1 font-mono" disabled={mutation.isPending}>
  ...
  return (
    <Link href={`/monitors/${monitor.id}`}>
      <div className={`group relative rounded-xl border p-5 cursor-pointer transition-all hover:shadow-lg hover:-translate-y-0.5 ${
        isDown ? "border-red-500/30 bg-red-500/5 hover:border-red-500/50"
        : isUp ? "border-green-500/20 bg-card/50 hover:border-green-500/40"
        : "border-border bg-card/50 hover:border-border/80"
      }`}>
        <div className="absolute top-4 right-4 flex items-center gap-2">
          <div className="relative flex items-center">
            {isUp && <span className="absolute inline-flex h-3 w-3 rounded-full bg-green-500 opacity-75 animate-ping" />}
            {isDown && <span className="absolute inline-flex h-3 w-3 rounded-full bg-red-500 opacity-75 animate-ping" />}
            <span className={`relative inline-flex h-3 w-3 rounded-full ${isUp ? "bg-green-500" : isDown ? "bg-red-500" : "bg-gray-500"}`} />
          </div>
          <span className={`text-xs font-mono font-bold ${isUp ? "text-green-500" : isDown ? "text-red-500" : "text-gray-500"}`}>
            {monitor.status.toUpperCase()}
          </span>
        </div>
        <div className="pr-20">
          <h3 className="font-semibold text-foreground text-base group-hover:text-primary transition-colors truncate">{monitor.name}</h3>
        ...
        <Card className="bg-card/50 border-border">
          ...
          <CardContent>
            {statsLoading ? <Skeleton className="h-8 w-24" /> : (
              <div className="text-2xl font-bold font-mono">{stats?.overallUptimePct?.toFixed(2) ?? "0.00"}%</div>
            )}
          </CardContent>
        </Card>
